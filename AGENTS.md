...
- ONE Cloudflare Cron Trigger `* * * * *` (wrangler.toml). `src/worker/cron.ts` runs every minute and dispatches GitHub Actions `workflow_dispatch` by **UTC minute alignment**:
  - every-5min `min%5==0`, every-10min `%10`, every-15min `%15`, every-30min `%30`, daily at 02:30/13:30/15:30/19:30/21:30 UTC (one slot per job family — 02:30 distribution+brief, 13:30 summary+neodove, 15:30 snatch, 19:30 baseline, 21:30 retention; the workflow gates each job on `inputs.slot`, `all` = manual runs everything); neodove-refresh runs natively in-worker every 5 min.
