...
- **Telecalling**: `Telecaller.assignEstimateFollowUps` (renamed from `active`, migration 0015) marks conversion specialists — only they get estimate follow-up assignments. Creator-first assignment still lets the lead-gen creator close their own estimate. Score ledger `TelecallerScoreEvent` (close slabs ₹0–1L 50 / ₹1–2.5L 75 / ₹2.5–5L 100 / ≥₹5L 200 split 50/50 generator/closer — halves 25/37/38/50/100; −10 remark / −15 legacy snatch / −20 retired decline). EOD run skips non-working days (zero NeoDove calls), shields 2+-attempt/connected effort per customer, honors dated next steps (`Estimate.nextStepDate`, migration 0028; future/today protects, past-due reads red), accepts `?dry=1` preview.
