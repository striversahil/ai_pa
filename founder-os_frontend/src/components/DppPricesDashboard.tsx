      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-zinc-800 pb-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-zinc-400 mt-0.5">Price quotes parsed from DPP WhatsApp messages</p>
          </div>
        </div>
        <button
          onClick={fetchData}
          className="flex items-center gap-2 px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-medium text-sm transition-all duration-200 cursor-pointer shadow-lg shadow-emerald-600/25 border-0"
        >
          <span>🔄</span> Refresh
        </button>
      </div>
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-zinc-900 border border-zinc-800/80 rounded-xl p-5">
              <span className="text-xs text-zinc-400 block mb-1">Total Quotes Parsed</span>
              <span className="text-2xl font-bold text-white block">{data.totalQuotes}</span>
            </div>
            <div className="bg-zinc-900 border border-zinc-800/80 rounded-xl p-5">
              <span className="text-xs text-zinc-400 block mb-1">Distinct Items</span>
              <span className="text-2xl font-bold text-white block">{data.distinctItems}</span>
            </div>
            <div className="bg-zinc-900 border border-zinc-800/80 rounded-xl p-5">
              <span className="text-xs text-zinc-400 block mb-1">Latest Quote</span>
              <span className="text-2xl font-bold text-emerald-400 block">
                {data.lastQuote ? `₹${data.lastQuote.price.toLocaleString()}` : "—"}
              </span>
              {data.lastQuote && <span className="text-xs text-zinc-500 block mt-1">{data.lastQuote.item}</span>}
            </div>
            <div className="bg-zinc-900 border border-zinc-800/80 rounded-xl p-5">
              <span className="text-xs text-zinc-400 block mb-1">Last Updated</span>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
            <div className="border-b border-zinc-800 px-5 py-4">
              <h3 className="text-lg font-bold text-white">Item Price Tracker</h3>
              <p className="text-xs text-zinc-500 mt-0.5">Latest average unit price per item</p>
            </div>
            {data.items.length === 0 ? (
              <div className="px-5 py-10 text-center text-sm text-zinc-500 italic">
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-zinc-800/60">
                  {data.items.map((item) => (
                    <tr key={item.itemName} className="hover:bg-zinc-800/30 transition-colors">
                      <td className="px-5 py-3 font-semibold text-zinc-200">{item.itemName}</td>
                      <td className="px-5 py-3 text-zinc-400">{item.timesQuoted}</td>
