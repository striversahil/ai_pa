...
export default function DppPricesDashboard() {
 ...
 <p className="text-sm text-[var(--text-tertiary)] mt-0.5">Price quotes parsed from DPP WhatsApp messages</p>
 </div>
 </div>
 <button
 onClick={fetchData}
 ...
 <span className="text-xs text-[var(--text-tertiary)] block mb-1">Total Quotes Parsed</span>
 <span className="text-2xl font-bold text-[var(--text-primary)] block">{data.totalQuotes}</span>
 </div>
 <div className="bg-[var(--bg-card)] border border-[var(--border-card)] rounded-xl p-5">
 <span className="text-xs text-[var(--text-tertiary)] block mb-1">Distinct Items</span>
 <span className="text-2xl font-bold text-[var(--text-primary)] block">{data.distinctItems}</span>
 </div>
 <div className="bg-[var(--bg-card)] border border-[var(--border-card)] rounded-xl p-5">
 <span className="text-xs text-[var(--text-tertiary)] block mb-1">Latest Quote</span>
 ...
 {data.lastQuote && <span className="text-xs text-[var(--text-tertiary)] block mt-1">{data.lastQuote.item}</span>}
 </div>
 <div className="bg-[var(--bg-card)] border border-[var(--border-card)] rounded-xl p-5">
 <span className="text-xs text-[var(--text-tertiary)] block mb-1">Last Updated</span>
 <span className="text-sm font-bold text-[var(--text-primary)] block mt-1.5">
 ...
 <h3 className="text-lg font-bold text-[var(--text-primary)]">Item Price Tracker</h3>
 <p className="text-xs text-[var(--text-tertiary)] mt-0.5">Latest average unit price per item</p>
 </div>
 {data.items.length === 0 ? (
 <div className="px-5 py-10 text-center text-sm text-[var(--text-tertiary)] italic">
 No quotes yet. When DPP sends <code>item — price</code> messages, they appear here automatically.
 </div>
 ) : (
 <table className="w-full text-sm">
 <thead>
 <tr className="text-left text-xs text-[var(--text-tertiary)] uppercase tracking-wider border-b border-[var(--border-card)]">
 ...
 {data.items.map((item) => (
 <tr key={item.itemName} className="hover:bg-black/5 dark:hover:bg-white/10 transition-colors">
 <td className="px-5 py-3 font-semibold text-[var(--text-primary)]">{item.itemName}</td>
 <td className="px-5 py-3 text-[var(--text-tertiary)]">{item.timesQuoted}</td>
