...
const statusColor: Record<string, string> = {
  ...
  draft: "text-zinc-300 bg-zinc-500/10 border-zinc-500/30",
...
export default function WhatsAppMarketingDashboard() {
  ...
  if (loading) return <div className="p-8 text-zinc-400">Loading marketing dashboard…</div>;
  if (error && !data) return <div className="p-8 text-rose-400">{error}</div>;

  const k = data?.kpis;

  const inputCls = "w-full rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-zinc-200 outline-none focus:border-sky-500";
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-zinc-100">WhatsApp Marketing</h1>
      ...
      {k && (
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
          ...
          <Kpi label="Draft" value={k.draft} accent="text-zinc-300" />
      ...
      {showNewCampaign && (
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-5 space-y-4">
          ...
          <button onClick={handleCreate} className="rounded-lg bg-sky-600 px-4 py-2 text-sm font-medium text-white hover:bg-sky-500">
            Create Campaign
          </button>
        </div>
      )}
        ...
        {data?.campaigns.map((c) => (
          <div key={c.id} className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4">
            <div className="flex items-start justify-between gap-4">
              <button className="text-left" onClick={() => openCampaign(c)}>
                <div className="flex items-center gap-2">
                  ...
                  {!c.enabled && <span className="rounded-full border border-zinc-600 px-2 py-0.5 text-[10px] text-zinc-400">disabled</span>}
                </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button onClick={() => handleRun(c.id)} disabled={runningId === c.id} className="rounded-lg border border-zinc-700 px-3 py-1.5 text-xs text-zinc-300 hover:border-sky-500 hover:text-sky-300 disabled:opacity-50">
                  {runningId === c.id ? "Running…" : "Run Now"}
                </button>
                <button onClick={() => handleToggleStatus(c)} className="rounded-lg border border-zinc-700 px-3 py-1.5 text-xs text-zinc-300 hover:border-amber-500 hover:text-amber-300">
                  {c.status === "active" ? "Pause" : "Activate"}
                </button>
                <button onClick={() => handleDelete(c)} className="rounded-lg border border-zinc-700 px-3 py-1.5 text-xs text-zinc-400 hover:border-rose-500 hover:text-rose-300">
                  Delete
                </button>
              </div>
            </div>
            <div className="mt-3 grid grid-cols-6 gap-2 text-center">
              ...
              <MiniStat label="Pending" value={c.stats.pending} cls="text-zinc-400" />
            </div>
          </div>
        ))}
      </div>
      ...
      {selectedCampaign && (
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-5 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-medium text-zinc-100">{selectedCampaign.name} — Leads</h2>
            <button onClick={() => setSelectedCampaign(null)} className="text-sm text-zinc-400 hover:text-zinc-200">Close</button>
          </div>

          <div className="space-y-2">
            <label className={labelCls}>Paste leads (CSV: phone,name,attrs… or JSON array)</label>
            <textarea
              className="w-full rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-zinc-200 outline-none focus:border-sky-500"
              rows={5}
              value={csvText}
              onChange={(e) => setCsvText(e.target.value)}
              placeholder={"phone,name\n919876543210,John Doe\n919111222333,Jane Smith"}
            />
            <button onClick={handleUploadLeads} className="rounded-lg bg-emerald-600 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-500">
              Upload Leads
            </button>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {leadsLoading && (
                  <tr><td colSpan={5} className="py-3 text-zinc-500">Loading leads…</td></tr>
                )}
                {!leadsLoading && leads.length === 0 && (
                  <tr><td colSpan={5} className="py-3 text-zinc-500">No leads yet.</td></tr>
                )}
                {leads.map((l) => (
                  <tr key={l.id} className="border-t border-zinc-800/50">
                    <td className="py-2 font-mono text-zinc-300">{l.phoneNumber}</td>
                    <td className="py-2 text-zinc-300">{l.name || "—"}</td>
                    <td className={`py-2 ${leadStatusColor[l.status] || "text-zinc-400"}`}>{l.status}</td>
                    <td className="py-2 max-w-[200px] truncate text-zinc-500">{l.error || "—"}</td>
                    <td className="py-2 text-zinc-500">{l.sentAt ? new Date(l.sentAt).toLocaleString() : "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {data?.recentRuns && data.recentRuns.length > 0 && (
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-5">
          <h2 className="mb-3 text-lg font-medium text-zinc-100">Recent Runs</h2>
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs text-zinc-500">
            ...
            <tbody>
              {data.recentRuns.map((r) => (
                <tr key={r.id} className="border-t border-zinc-800/50">
                  <td className="py-2 text-zinc-400">{new Date(r.startedAt).toLocaleString()}</td>
                  <td className={`py-2 ${r.status === "failed" ? "text-rose-400" : r.status === "completed" ? "text-emerald-300" : "text-zinc-300"}`}>{r.status}</td>
                  <td className="py-2 text-zinc-400">{r.total}</td>
...
function Kpi({ label, value, accent = "text-zinc-100" }: { label: string; value: number; accent?: string }) {
  return (
...
function MiniStat({ label, value, cls = "text-zinc-300" }: { label: string; value: number; cls?: string }) {
  return (
