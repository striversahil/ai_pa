...
export default function Automations({ slug, onNavigate }: AutomationsProps) {
 ...
 className={`bg-[var(--bg-card)] border rounded-xl p-5 transition-all duration-200 ${isDraggable ? "cursor-grab active:cursor-grabbing" : ""
 } ${isDropTarget ? "border-indigo-500 ring-1 ring-indigo-500/50" : "border-[var(--border-card)] hover:border-indigo-500/40"} ${isDragging ? "opacity-50" : ""}`}
 >
 <div className="flex items-start justify-between gap-3">
 <div className="min-w-0">
 <h3 className="font-bold text-[var(--text-primary)] truncate">{a.name}</h3>
 <code className="text-[11px] text-indigo-600 dark:text-indigo-400 font-mono">{a.slug}</code>
 </div>
 <div className="flex items-center gap-2 shrink-0">
 {isDraggable && (
 <span className="text-[var(--text-tertiary)] select-none text-lg leading-none" title="Drag to reorder">⠿</span>
 )}
 <button
 ...
 : "bg-black/5 dark:bg-white/10 text-[var(--text-tertiary)] border-black/15 dark:border-white/15 hover:text-[var(--text-primary)] hover:border-zinc-300 dark:hover:border-zinc-600"
 }`}
 >
 ...
 : "bg-black/5 dark:bg-white/10 text-[var(--text-tertiary)] border-black/15 dark:border-white/15"
 }`}
 >
 ...
 {a.description && (
 <p className="text-xs text-[var(--text-tertiary)] mt-2 line-clamp-2">{a.description}</p>
 )}

 <div className="flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-[var(--text-tertiary)] mt-3">
 ...
 ? "bg-black/5 dark:bg-white/10 hover:bg-black/10 dark:hover:bg-white/10 text-[var(--text-secondary)]"
 ...
 <div className="border-b border-[var(--border-card)] pb-5">
 ...
 <code className="text-[var(--text-secondary)]">src/automations/&lt;slug&gt;/</code>. Pin your favourite dashboards for quick access.
 </p>
 </div>
 ...
 className="flex items-center gap-2 px-4 py-2 rounded-lg bg-black/5 dark:bg-white/10 hover:bg-black/10 dark:hover:bg-white/10 font-medium text-sm transition-all duration-200 cursor-pointer border-0"
 >
 ...
 : "bg-black/5 dark:bg-white/10 hover:bg-black/10 dark:hover:bg-white/10 text-[var(--text-primary)]"
 }`}
 >
 ...
 📌 Pinned <span className="text-[var(--text-tertiary)] normal-case font-medium">— drag to reorder</span>
 </h2>
