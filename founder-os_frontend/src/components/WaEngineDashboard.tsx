      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 border-b border-zinc-200 dark:border-zinc-800 pb-5">
        <div>
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
            session <code className="text-zinc-700 dark:text-zinc-300 font-mono">{data?.meta?.sessionName ?? "—"}</code>
            {data?.meta?.generatedAt && ` · updated ${new Date(data.meta.generatedAt).toLocaleTimeString()}`}
            {typeof data?.meta?.cacheAgeSec === "number" && (
              <span className="text-zinc-600 dark:text-zinc-500"> · WA Engine snapshot {data.meta.cacheAgeSec}s old (cron refreshes every 5m)</span>
            )}
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex items-center gap-1.5 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg p-1.5 text-xs">
            <span className="text-[10px] text-zinc-600 dark:text-zinc-500 font-bold uppercase tracking-wider pl-1">Window:</span>
            <select
              value={windowDays}
      ...
      {!loading && !error && data && (
        <>
            ...
            <div className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 shadow-md">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                <div className="col-span-2 md:col-span-3 flex items-center gap-4">
                  <div className="w-11 h-11 rounded-full bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-lg font-bold text-zinc-900 dark:text-white">
                    {(data.diagnostics.pushName ?? "?").charAt(0).toUpperCase()}
                  </div>
                  <div>
                    <div className="text-zinc-900 dark:text-white font-semibold text-sm">{data.diagnostics.pushName ?? "—"}</div>
                    <div className="text-zinc-500 dark:text-zinc-400 text-xs font-mono">{data.diagnostics.accountId ?? "—"}</div>
                  </div>
                </div>
          ...
          {data.uptimeByDay && data.uptimeByDay.length > 0 && (
            <div className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 shadow-md">
              <h3 className="text-zinc-900 dark:text-white font-bold text-base mb-4">Uptime by Day</h3>
              <div className="space-y-2">
                {data.uptimeByDay.map((d) => (
                  <div key={d.date} className="flex items-center gap-3">
                    <span className="w-14 text-zinc-500 dark:text-zinc-400 text-[10px] font-mono whitespace-nowrap">{d.date}</span>
                    <div className="flex-1 h-4 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded overflow-hidden">
                      <div
                        className={`h-full ${d.uptime >= 99 ? "bg-emerald-500" : d.uptime >= 90 ? "bg-amber-500" : "bg-rose-500"}`}
                        style={{ width: `${d.uptime}%` }}
                      />
                    </div>
                    <span className="w-14 text-right text-zinc-700 dark:text-zinc-300 text-[10px] font-mono">{d.uptime}%</span>
                  </div>
                ))}
              </div>
            </div>
...
function DiagField({ label, value }: { label: string; value: string | null }) {
  return (
    <div className="bg-zinc-50 dark:bg-zinc-950 border border-zinc-200/60 dark:border-zinc-800/60 rounded-lg px-3.5 py-2.5">
      <div className="text-zinc-600 dark:text-zinc-500 text-[9px] font-bold uppercase tracking-wider mb-0.5">{label}</div>
      <div className="text-zinc-800 dark:text-zinc-200 text-xs font-mono truncate">{value ?? "—"}</div>
    </div>
  );
}
...
function MetricTile({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="bg-zinc-50 dark:bg-zinc-950 border border-zinc-200/60 dark:border-zinc-800/60 rounded-lg px-3.5 py-3">
      <div className="text-zinc-600 dark:text-zinc-500 text-[9px] font-bold uppercase tracking-wider mb-1">{label}</div>
      <div className="text-zinc-900 dark:text-zinc-100 text-xl font-extrabold font-mono">{value}</div>
    </div>
  );
}
