...
export default function WaEngineDashboard() {
 ...
 session <code className="text-[var(--text-secondary)] font-mono">{data?.meta?.sessionName ?? "—"}</code>
 {data?.meta?.generatedAt && ` · updated ${new Date(data.meta.generatedAt).toLocaleTimeString()}`}
 {typeof data?.meta?.cacheAgeSec === "number" && (
 <span className="text-[var(--text-tertiary)]"> · WA Engine snapshot {data.meta.cacheAgeSec}s old (cron refreshes every 5m)</span>
 )}
 </p>
 </div>
 <div className="flex flex-wrap items-center gap-2">
 <div className="flex items-center gap-1.5 bg-[var(--bg-card)] border border-[var(--border-card)] rounded-lg p-1.5 text-xs">
 <span className="text-[10px] text-[var(--text-tertiary)] font-bold uppercase tracking-wider pl-1">Window:</span>
 <select
 value={windowDays}
 ...
 {data.diagnostics && (
 <div className="bg-[var(--bg-card)] border border-[var(--border-card)] rounded-xl p-6 shadow-md">
 <div className="flex items-center justify-between mb-4">
 <h3 className="text-[var(--text-primary)] font-bold text-base">Connected Account & Engine</h3>
 ...
 <div className="w-11 h-11 rounded-full bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-lg font-bold text-[var(--text-primary)]">
 {(data.diagnostics.pushName ?? "?").charAt(0).toUpperCase()}
 </div>
 <div>
 <div className="text-[var(--text-primary)] font-semibold text-sm">{data.diagnostics.pushName ?? "—"}</div>
 <div className="text-[var(--text-tertiary)] text-xs font-mono">{data.diagnostics.accountId ?? "—"}</div>
 </div>
 </div>
 ...
 <div className="bg-[var(--bg-card)] border border-[var(--border-card)] rounded-xl p-6 shadow-md">
 <h3 className="text-[var(--text-primary)] font-bold text-base mb-4">Uptime by Day</h3>
 <div className="space-y-2">
 {data.uptimeByDay.map((d) => (
 <div key={d.date} className="flex items-center gap-3">
 <span className="w-14 text-[var(--text-tertiary)] text-[10px] font-mono whitespace-nowrap">{d.date}</span>
 <div className="flex-1 h-4 bg-[var(--bg-input)] border border-[var(--border-card)] rounded overflow-hidden">
 <div
 className={`h-full ${d.uptime >= 99 ? "bg-emerald-500" : d.uptime >= 90 ? "bg-amber-500" : "bg-rose-500"}`}
 style={{ width: `${d.uptime}%` }}
 />
 </div>
 <span className="w-14 text-right text-[var(--text-secondary)] text-[10px] font-mono">{d.uptime}%</span>
 </div>
 ))}
 </div>
 </div>
 ...
 <div className="bg-[var(--bg-card)] border border-[var(--border-card)] rounded-xl p-6 shadow-md">
 <div className="flex items-center justify-between mb-4">
 <h3 className="text-[var(--text-primary)] font-bold text-base">Platform Health</h3>
...
function DiagField({ label, value }: { label: string; value: string | null }) {
 return (
 <div className="bg-[var(--bg-input)] border border-[var(--border-card)] rounded-lg px-3.5 py-2.5">
 <div className="text-[var(--text-tertiary)] text-[9px] font-bold uppercase tracking-wider mb-0.5">{label}</div>
 <div className="text-[var(--text-primary)] text-xs font-mono truncate">{value ?? "—"}</div>
 </div>
 );
}
...
function MetricTile({ label, value }: { label: string; value: string | number }) {
 return (
 <div className="bg-[var(--bg-input)] border border-[var(--border-card)] rounded-lg px-3.5 py-3">
 <div className="text-[var(--text-tertiary)] text-[9px] font-bold uppercase tracking-wider mb-1">{label}</div>
 <div className="text-[var(--text-primary)] text-xl font-extrabold font-mono">{value}</div>
 </div>
 );
}
