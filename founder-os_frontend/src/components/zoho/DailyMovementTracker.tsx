      ...
      <div className="bg-zinc-50/40 dark:bg-zinc-950/40 border border-zinc-200/80 dark:border-zinc-800/80 p-3 rounded-xl">
        ...
        {salesOrdersTodayOrders && salesOrdersTodayOrders.length > 0 && (
          <>
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto divide-y divide-zinc-200/40 dark:divide-zinc-800/40 pr-1 scrollbar-thin mb-3">
        ...
        {/* Estimate status transitions today (declined / new / accepted-vs-baseline). */}
        {(movement.accepted.length > 0 || movement.declined.length > 0 || movement.newCreated.length > 0) && (
          <>
            <span className="text-[9px] text-zinc-600 dark:text-zinc-500 font-bold uppercase tracking-wider block mb-1.5">Estimate Movements Today</span>
        ...
        {(!salesOrdersTodayOrders || salesOrdersTodayOrders.length === 0) && movement.accepted.length === 0 && movement.declined.length === 0 && movement.newCreated.length === 0 && (
          <div className="text-xs text-zinc-600 dark:text-zinc-500 italic py-2 text-center">
            {salesOrdersTodayOrders === null ? "Loading sales orders…" : "No sales orders or estimate movements today. Auto-syncs every 15 minutes."}
          </div>
        )}
      </div>
    </div>
