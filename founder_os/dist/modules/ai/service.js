...
const summarizeConversation_1 = require("./prompts/summarizeConversation");
const generateBrief_1 = require("./prompts/generateBrief");
const generateDailySummary_1 = require("./prompts/generateDailySummary");
const answerFounderQuestion_1 = require("./prompts/answerFounderQuestion");
...
const openai = !isMockLLM
    ? new openai_1.default({
        apiKey: config_1.config.LLM_API_KEY,
        baseURL: config_1.config.LLM_BASE_URL,
    })
    : null;
class AIService {
     ...
     */
    static async summarizeConversation(chatName, messages) {
        const startTime = Date.now();
        logger_1.logger.info({ chatName, isMocked: isMockLLM }, 'AIService: digesting conversation');
        if (isMockLLM) {
            // Simulate LLM delay
        ...
        try {
            const response = await openai.chat.completions.create({
                ...
                messages: [
                    { role: 'system', content: summarizeConversation_1.summarizeConversationPrompt },
                    { role: 'user', content: `Chat Name: ${chatName}\n\nMessages:\n${messages.map((m) => `[${m.timestamp.toISOString()}] ${m.sender}: ${m.body}`).join('\n')}` },
                ],
                temperature: 0.1,
            });
            const latency = Date.now() - startTime;
            logger_1.logger.info({ latency }, 'Real LLM summarization completed');
            const rawContent = response.choices[0]?.message?.content || '';
            const cleanJson = this.cleanJsonString(rawContent);
            return JSON.parse(cleanJson);
        }
        catch (err) {
            logger_1.logger.error({ error: err.message }, 'Real LLM summarization failed');
            throw err;
        }
    }
    /**
    ...
    static async generateFounderBrief(context) {
        const startTime = Date.now();
        logger_1.logger.info({ isMocked: isMockLLM }, 'AIService: generating morning brief');
        if (isMockLLM) {
            await new Promise((r) => setTimeout(r, 600));
            const now = new Date();
...
`;
        ...
        try {
            const systemPrompt = generateBrief_1.generateBriefPrompt
            ...
            logger_1.logger.error({ error: err.message }, 'Real LLM brief generation failed');
            throw err;
        }
    }
    /**
    ...
    static async generateDailySummary(context) {
        logger_1.logger.info({ isMocked: isMockLLM }, 'AIService: generating EOD summary');
        if (isMockLLM) {
            await new Promise((r) => setTimeout(r, 600));
            const now = new Date();
...
`;
        ...
        try {
            const systemPrompt = generateDailySummary_1.generateDailySummaryPrompt
            ...
            logger_1.logger.error({ error: err.message }, 'Real LLM summary generation failed');
            throw err;
        }
    }
    /**
     ...
     */
    static async answerFounderQuestion(question, context) {
        logger_1.logger.info({ question, isMocked: isMockLLM }, 'AIService: answering question');
        if (isMockLLM) {
...
Please let me know which of these you would like more detail on, sir.`;
        ...
        try {
            const systemPrompt = answerFounderQuestion_1.answerFounderQuestionPrompt
            ...
            logger_1.logger.error({ error: err.message }, 'Real LLM Q&A failed');
            throw err;
        }
    }
}
