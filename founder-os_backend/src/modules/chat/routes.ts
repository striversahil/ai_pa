...
export async function chatListChannels(store: ChatStore, me: MeResponse): Promise<ChatResult> {
  return json(200, await store.listChannels(me.user.id));
}
...
export async function chatCreateChannel(
  store: ChatStore,
  me: MeResponse,
  payload: { name?: string; description?: string | null; category?: string | null },
): Promise<ChatResult> {
...
export async function chatListMessages(
  ...
  if (!(await store.canAccessChannel(channelId, me.user.id))) return err("You don't have access to this channel");
...
export async function chatSendMessage(
  ...
  const message = await store.createMessage({ channelId, senderId: me.user.id, body, attachments });
  return {
    status: 201,
    body: message,
...
export async function chatUpdateMessage(
  ...
  if (!(await store.canAccessChannel(existing.channelId, me.user.id))) return err("You don't have access to this channel");
