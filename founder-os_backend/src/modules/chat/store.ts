...
export interface ChatStore {
  listChannels(userId: string): Promise<ChatChannel[]>;
  createChannel(input: { name: string; description: string | null; category: string | null; createdBy: string }): Promise<ChatChannel>;
  ...
  createMessage(input: { channelId: string; senderId: string; body: string; attachments?: ChatAttachment[] }): Promise<ChatMessage>;
...
class MemoryChatStore implements ChatStore {
  ...
  async createMessage(input) {
    const msg: ChatMessage = { id: this.seq++, channelId: input.channelId, senderId: input.senderId, senderName: input.senderId, senderPicture: null, body: input.body, createdAt: new Date().toISOString(), editedAt: null, deletedAt: null, attachments: input.attachments ?? [] };
...
class D1ChatStore implements ChatStore {
  ...
  async listChannels(userId: string) {
    const rows = await this.db
      .prepare(
        "SELECT * FROM chat_channel c WHERE c.type = 'channel' OR EXISTS (SELECT 1 FROM chat_member m WHERE m.channelId = c.id AND m.userId = ?) ORDER BY c.createdAt ASC",
      )
      .bind(userId)
      .all();
  ...
  async canAccessChannel(channelId: string, userId: string) {
    const row = await this.db
      .prepare(
        "SELECT 1 FROM chat_channel c WHERE c.id = ? AND (c.type = 'channel' OR EXISTS (SELECT 1 FROM chat_member m WHERE m.channelId = c.id AND m.userId = ?))",
      )
    ...
    const rows = before
      ? await this.db
          .prepare(
            `SELECT m.*, u.name AS senderName, u.picture AS senderPicture
             FROM chat_message m JOIN auth_user u ON u.id = m.senderId
      ...
      : await this.db
          .prepare(
            `SELECT m.*, u.name AS senderName, u.picture AS senderPicture
             FROM chat_message m JOIN auth_user u ON u.id = m.senderId
             WHERE m.channelId = ? ORDER BY m.id DESC LIMIT ?`,
          )
  ...
  async getMessage(id: number) {
    const row = await this.db
      .prepare(
        `SELECT m.*, u.name AS senderName, u.picture AS senderPicture
         FROM chat_message m JOIN auth_user u ON u.id = m.senderId WHERE m.id = ?`,
      )
      .bind(id)
      .first();
  ...
  async createMessage(input) {
    await this.db
      .prepare("INSERT INTO chat_message (channelId, senderId, body, createdAt, attachments) VALUES (?, ?, ?, ?, ?)")
      .bind(input.channelId, input.senderId, input.body, new Date().toISOString(), JSON.stringify(input.attachments ?? []))
      .run();
    const row = await this.db
      .prepare(
        `SELECT m.*, u.name AS senderName, u.picture AS senderPicture
