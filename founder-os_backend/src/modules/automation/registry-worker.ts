...
const RULES: Record<string, Partial<AutomationDefinition>> = {
  ...
  'enquiry-tracker': {
    id: 'enquiry-tracker', name: 'Sales Enquiries', description: 'Live sales pipeline: daily enquiries + threaded comments, persisted and pushed to open dashboards.', type: 'handler', trigger: { type: 'manual' }, scope: 'enquiry-tracker', enabled: true,
  },
  'enquiry-procurement': {
    id: 'enquiry-procurement', name: 'Sourcing Queue (Pre-Sale Vendor Rates)', description: 'Pending-only vendor-rate queue: items awaiting rates, client PII hidden. No daily pipeline — just what needs rates now.', type: 'handler', trigger: { type: 'manual' }, scope: 'procurement', enabled: true,
  },
  'enquiry-management': {
    id: 'enquiry-management', name: 'Management Review', description: 'Pending-only rate review: rated items awaiting markup + finalize. No daily pipeline — just what needs a decision now.', type: 'handler', trigger: { type: 'manual' }, scope: 'mis', enabled: true,
  },
  'dpp-prices-dashboard': {
    ...
    actions: [{ type: 'custom:storePrices' }], config: { dppChatId: '' }, scope: 'dpp', enabled: true,
  },
  'enterprise-operations-analytics': {
    id: 'enterprise-operations-analytics', name: 'Enterprise Operations & Order Analytics', description: '18-Point Complete Enterprise Supply Chain Analysis dashboard.', type: 'handler', trigger: { type: 'schedule', cron: '*/30 * * * *' }, scope: 'enterprise-ops', enabled: true,
  },
  'eod-summary': {
  ...
  'telecalling-agent-analysis': {
    ...
    config: { sheetUrl: 'https://docs.google.com/spreadsheets/d/1OsQevXQpPT1x2iJgcg0lgUcOInxjZh3tvfNjxAbcENs/edit', range: 'A1:Z1000', maxDailySO: 50 }, scope: 'sheet-analysis', enabled: true,
  },
  ...
  'wa-engine-monitor': {
    id: 'wa-engine-monitor', name: 'WA Engine Pro Monitor', description: 'Every 5 minutes, check WA Engine Pro API connectivity and audit sustained outages.', type: 'handler', trigger: { type: 'schedule', cron: '*/5 * * * *' }, config: { windowDays: 7 }, scope: 'wa-engine', enabled: true,
  },
  'whatsapp-marketing': {
  ...
  'digital-marketing': {
    id: 'digital-marketing', name: 'Digital Marketing', description: 'Daily BUI digital marketing taskbar: account sheet, weekly collateral posts (FB/IG/GBP/LI) and daily numeric inputs (Meta Ads, B2B, Whatsapp, Email) with MIS controller and KRA/KPI.', type: 'handler', trigger: { type: 'schedule', cron: '30 3 * * *' }, scope: 'digital-marketing', enabled: true,
  },
  'whatsapp-autopilot': {
...
/** slug → permission scope, from the RULES map (mirrors rule.json `scope`).
 *  zoho-sent-analyzer is a GH-Actions-heavy automation (not worker-registered)
 *  but still serves a dashboard, so its scope is declared here too. */
export const AUTOMATION_SCOPES: Record<string, string> = {
  ...Object.fromEntries(
    Object.entries(RULES).flatMap(([slug, r]) => (r.scope ? [[slug, r.scope]] : [])),
  ),
