...
- meaningful_update MUST be true if THIS comment clearly mentions a specific follow-up date, day, or time (e.g. "call after 15 August", "he said call after 15th", "will follow up on Monday", "call after 2 days"). A customer-stated or committed follow-up date is a meaningful next step. EXCEPTION: a date alone is not progress — if the comment states the customer could not be reached (not answering / unable to connect / unreachable / switched off / call not picked up), the agent's own retry plan is NOT a meaningful update even when it names a retry date: set meaningful_update=false and not_answering=true.
