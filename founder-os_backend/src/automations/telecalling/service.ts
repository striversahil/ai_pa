  ...
  if (agentFilter) {
    ...
    const followUps = followUpEsts.map((e) => {
      ...
      return {
        ...
        lastCommentDate,
        staleHours,
        // Lead details: prefer the per-estimate extraction (stored by the GH
        // runner from Zoho comments); fall back to the matched enquiry.
        enquiryNumber: (e as any).enquiryNumber ?? enquiry?.enquiryNumber ?? null,
        sourceLead: (e as any).sourceLead ?? enquiry?.sourceLead ?? null,
        location: (e as any).location ?? enquiry?.location ?? null,
        contactName: (e as any).contactName ?? enquiry?.contactName ?? null,
        contactPhone: (e as any).contactPhone ?? enquiry?.contactPhone ?? null,
